...
class TestNetworkGraph < ActiveRecord::Base
  ...
  def nodes_and_edges
    ...
    graph["sites"].each do |site|
      ...
      self.generate_for_collection(iteration_helper, collection, site.keys.last.to_s.singularize, 1, nil)
    end
  ...
  def generate_for_collection(iteration_helper, collection, node_type, node_level, parent_id)
    collection.each do |object|
      ...
      iteration_helper.add_node(node)
      ...
      if parent_id.present?
        ...
        iteration_helper.add_edge(edge)
      end
      ...
      if nested_collection.present?
        ...
        self.generate_for_collection(iteration_helper, nested_collection, object.keys.last.to_s.singularize, next_node_level, previous_parent_id)
      end
    end
  end
end
